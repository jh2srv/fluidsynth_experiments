...
class MidiInputHandler(object):
    ...
    def __call__(self, event, data=None):
        ...
        if message[0] == 144:
            ...
            self._executor.submit(self._fsynth.noteon, 0, message[1], message[2])
            return

        if message[0] == 128:
            # self._fsynth.noteoff(0, message[1])
            self._executor.submit(self._fsynth.noteoff, 0, message[1])
            return
        if message[0] == 176:
            # self._fsynth.cc(0, 1, message[2]) # 1 = vibrato
            self._executor.submit(self._fsynth.cc, 0, 1, message[2])
            return

        if message[0] == 224:
            value = 130*(message[2] - 64)
            # self._fsynth.pitch_bend(0, value)
            self._executor.submit(self._fsynth.pitch_bend, 0, value)
            return

fsynth = fluidsynth.Synth(samplerate = 44100)
fsynth.setting('audio.alsa.device','hw:1')
fsynth.setting("audio.period-size",64 )
fsynth.setting("audio.periods", 10)
